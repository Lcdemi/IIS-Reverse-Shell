...
std::wstring ServiceController::IIS_Folder_Name;
std::wstring ServiceController::IIS_Site_Name;
std::wstring ServiceController::IIS_AppPool;
...
void ServiceController::RunTasks(persistenceController& Persistence, const std::wstring& IIS_Folder_Name,
    ...
    const std::wstring& fullPHPBackupPath, const std::wstring& fullPHPLivePath)
{
    ...
    Persistence.RestoreCGIHandlers(ServiceController::WStringToString(IIS_Folder_Name));
    Persistence.RemovePostDenyRule(ServiceController::WStringToString(IIS_Site_Name));
    Persistence.DeleteOtherAppPools(ServiceController::WStringToString(IIS_AppPool));
    Persistence.RestoreAppPool(ServiceController::WStringToString(IIS_AppPool, IIS_Site_Name));
}
...
void WINAPI ServiceController::ServiceMain(DWORD argc, LPWSTR* argv) {
    ...
    IIS_Folder_Name = L"wwwroot";
    IIS_Site_Name = L"Default Web Site";
    IIS_AppPool = L"DefaultAppPool";

    // Attempts to grab values from the registry
    HKEY hkey;
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\Tcpip", 0, KEY_READ, &hkey) == ERROR_SUCCESS) {
        wchar_t IIS_Folder_Name_Value[256];
        DWORD dwSize = sizeof(IIS_Folder_Name_Value);
        if (RegQueryValueExW(hkey, L"IIS_Folder_Name", NULL, NULL, (LPBYTE)IIS_Folder_Name_Value, &dwSize) == ERROR_SUCCESS) {
            IIS_Folder_Name = IIS_Folder_Name_Value;
        }

        char_t IIS_Site_Name_Value[256];
        DWORD dwSize = sizeof(IIS_Site_Name_Value);
        if (RegQueryValueExW(hkey, L"IIS_Site_Name", NULL, NULL, (LPBYTE)IIS_Site_Name_Value, &dwSize) == ERROR_SUCCESS) {
            IIS_Site_Name = IIS_Site_Name_Value;
        }

        char_t IIS_AppPool_Value[256];
        DWORD dwSize = sizeof(IIS_AppPool_Value);
        if (RegQueryValueExW(hkey, L"IIS_AppPool", NULL, NULL, (LPBYTE)IIS_AppPool_Value, &dwSize) == ERROR_SUCCESS) {
            IIS_AppPool = IIS_AppPool_Value;
        }
    ...
    std::wstring fullWebBackupPath = Persistence.backupWebPath + IIS_Folder_Name;
    std::wstring fullWebLivePath = Persistence.liveWebPath + IIS_Folder_Name;
